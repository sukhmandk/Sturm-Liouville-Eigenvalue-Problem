...
!
!    chebexps2 - this routine constructs the same data as chebexps, but packages
!      in a structure for convenience
!
!    chebpts - return the nodes of the n-point Chebyshev grid on the interval [a,b]
!
...
!      on each subinterval
!
!    chebadap2 - a rather specialized version of the chebadap routine which discretizes
!      a function of the form g(t) = f(\lambda,t) with \lambda fixed and specified
!      by the user
!
...
!
!    chebrefine_greedy - a specialized version of chebrefine which attempts to
!      maximum the size of the intervals in the decomposition of [a,b]
!
...
!
!    chebdiff - return the (n,n) Chebyshev spectral differentiaion matrix; that is,
!      the (n,n) matrix which takes the values of a polynomial of degree less than
!      or equal to n-1 at the n-point grid on [-1,1] to the values of its 
...
subroutine chebadapfun2(dlambda,t,val,userptr)
import c_ptr
double precision, intent(in)      :: t,dlambda
double precision, intent(out)     :: val
type(c_ptr)                       :: userptr
end subroutine
...
subroutine chebexps2(n,chebdata)
implicit double precision (a-h,o-z)
integer             :: n
type(chebexps_data) :: chebdata

!
!  Return the same data as chebexps, but packaged in a structure for convenience.
!
!  Input parameters:
!    n - the number of points in the Chebyshev grid
!
!  Output parameters:
!    chebdata - a structure containing the data returned by chebexps
!

chebdata%k = n 
call chebexps(chebdata%k,chebdata%xs,chebdata%whts,chebdata%u,chebdata%v,&
  chebdata%aintl,chebdata%aintr)
...
subroutine chebadap(ier,eps,a,b,fun,k,xs,u,nints,ab,userptr)
implicit double precision (a-h,o-z)
...
subroutine chebadap2(ier,eps,a,b,dlambda0,fun,k,xs,u,nints,ab,userptr)
implicit double precision (a-h,o-z)

double precision, intent(in)               :: eps,xs(k),u(k,k),a,b,dlambda0
integer, intent(in)                        :: k
integer, intent(out)                       :: nints
double precision, intent(out), allocatable :: ab(:,:)
...
!
!  This is a specialized version of chebadap which adaptively discretizes
!  a function depending on a parameter at a fixed value of the parameter.
!
!  That is, it given f(t,\lambda) and  \lambda_0, it adaptively discretizes
!  the function
!
!    g(t) = f(t,\lambda_0).
!
...
!    eps - the precision with which the function should be represented
!    (a,b) - the interval on which the function is given
!    dlambda0 - the fixed value of the parameter \lambda
!    fun - an external procedure conforming to the interface chebadapfun2
!      which returns the values of the function
!
!    k - the number of terms in the polynomial expansions
!    xs - the nodes of the k-point Clenshaw-Curtis quadrature returned by chebexps
...
do i=1,k
call fun(dlambda0,ts(i),vals(i),userptr)
end do
...
subroutine chebrefine(ier,eps,nints,ab,fun,k,xs,u,userptr)
implicit double precision (a-h,o-z)
...
subroutine chebrefine_greedy(ier,eps,nints,ab,fun,k,xs,u,userptr)
implicit double precision (a-h,o-z)
...
!  starting point.
!
!  This version of the routine uses a primitive greedy algorithm to try to
!  maximum the size of the intervals which result using a (very) primitive
!  greedy algorithm.  This routine is probably most useful in the event
!  that the function being discretized has a singularity at the point a.
!
!  Input parameters:
!
!    eps - the precision with which the function should be represented
...
!
!  Check to see if splitting  make sense
!
c0 = (a0+b0)/2
ifmore = 0

do while (ifmore .eq. 1) 

c00 = (9*c0+a0)/10
!c00 = (c0+9*a0)/10

ts  = xs*(b0-c00)/2 + (b0+c00)/2
do i=1,k
call fun(ts(i),vals(i),userptr)
end do
coefs = matmul(u,vals)
...
subroutine chebints(x,n,pols,dints)
implicit double precision (a-h,o-z)

integer :: n 
double precision :: pols(n+2),dints(n+1)

!
!  Evaluate the Chebyshev polynomials order 0 through n and their antiderivatives
!  (the antiderivatives which are zero at the left endpoint of the interval)
!  at a specified point using the standard 3-term recurrence relation. 
!
!  WARNING: the array of polynomials values passed by the user must be of
!  length n+2!!!
...
do i=2,n
dints(i+1) = 0.5d0 * (pols(i+2)/(i+1) - pols(i)/(i-1)) + (-1)**(i+1)/(i**2-1.0d0)
end do
...
!call plot_function(ifshow,filename,"","",k*nints,ts,vals)
...
subroutine chebpw_eval222(nints,ab,k,xscheb,vals,vals2,x,val,val2)
implicit double precision (a-h,o-z)

integer, intent(in)           :: nints,k
double precision, intent(in)  :: xscheb(k),ab(2,nints),vals(k,nints),vals2(k,nints)
double precision, intent(out) :: val,val2
...
! sum1 =su
! end do
sum1 = sum(bwhts*vals(:,int))
sum2 = sum(bwhts*vals2(:,int))
sum3 = sum(bwhts)
