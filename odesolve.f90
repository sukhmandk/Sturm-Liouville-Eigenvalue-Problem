...
integer, parameter, private  :: maxiters     = 4
integer, parameter, private  :: maxiterstrap = 4

interface

subroutine odefunction(t,y,yp,f,dfdy,dfdyp,userptr)
import c_ptr
...
subroutine ode_solve_ivp(ier,nints,ab,k,xscheb,chebintl,ys,yders,yder2s,odefun,userptr)
implicit double precision (a-h,o-z)
...
double precision, allocatable :: chebint2(:,:)
...
allocate(ys0(k),yders0(k),yder2s0(k),chebint2(k,k))
...
call ode_trap_ivp(ier,k,ts,odefun,ys(:,int),yders(:,int),yder2s(:,int),userptr)
...
do i=1,k
call odefun(ts(i),ys(i,int),yders(i,int),f,dfdy,dfdyp,userptr)
ps(i)    = -dfdyp
qs(i)    = -dfdy
fs(i)    = f-yder2s(i,int)
end do
...
hs(1)    = 0
hders(1) = 0
call ode_linear_ivp(a,b,k,xscheb,chebintl,chebint2,ps,qs,fs,hs,hders,hder2s)
...
do i=1,k
call odefun(ts(i),ys0(i),yders0(i),f,dfdy,dfdyp,userptr)
ps(i)    = -dfdyp
qs(i)    = -dfdy
fs(i)    = f-yder2s0(i)
end do
...
subroutine ode_solve_tvp(ier,nints,ab,k,xscheb,chebintr,ys,yders,yder2s,odefun,userptr)
implicit double precision (a-h,o-z)
...
double precision, allocatable :: ts(:),ps(:),qs(:),fs(:),amatr(:,:)
double precision, allocatable :: hs(:),hders(:),hder2s(:)
double precision, allocatable :: ys0(:),yders0(:),yder2s0(:),chebint2(:,:)
...
call ode_trap_tvp(ier,k,ts,odefun,ys0,yders0,yder2s0,userptr)
...
hs(k)    = 0
hders(k) = 0
call ode_linear_tvp(a,b,k,xscheb,chebintr,chebint2,ps,qs,fs,hs,hders,hder2s)
...
subroutine ode_solve_ivp_adap(ier,eps,nintsin,abin,k,xscheb,chebintl,ucheb,nints,ab,  &
   ys,yders,yder2s,odefun,ya,ypa,userptr)
implicit double precision (a-h,o-z)
...
!      ier = 4096   means that NaN or Inf was encounterd
!
!    ys - the values of the solution y of (2) at the Chebyshev nodes
!    yders - the values of the solution y' of (2) at the Chebyshev nodes
!      Chebyshev grid on [a,b]
...
double precision, allocatable :: ts(:),ps(:),qs(:),fs(:),amatr(:,:),chebint2(:,:)
double precision, allocatable :: hs(:),hders(:),hder2s(:)
...
call ode_trap_ivp(ier,k,ts,odefun,ys1,yders1,yder2s1,userptr)
...
do i=1,k
call odefun(ts(i),ys1(i),yders1(i),f,dfdy,dfdyp,userptr)
ps(i)    = -dfdyp
qs(i)    = -dfdy
fs(i)    = f-yder2s1(i)
end do
...
hs(1)    = 0
hders(1) = 0
call ode_linear_ivp(a0,b0,k,xscheb,chebintl,chebint2,ps,qs,fs,hs,hders,hder2s)
...
subroutine ode_solve_tvp_adap(ier,eps,nintsin,abin,k,xscheb,chebintr,ucheb,nints,ab,  &
   ys,yders,yder2s,odefun, yb,ypb,userptr)
implicit double precision (a-h,o-z)
...
double precision, allocatable :: ts(:),ps(:),qs(:),fs(:),amatr(:,:)
double precision, allocatable :: hs(:),hders(:),hder2s(:),chebint2(:,:)
...
call ode_trap_tvp(ier,k,ts,odefun,ys1,yders1,yder2s1,userptr)
if (ier .ne. 0) then
ifsplit = 1
goto 1000
endif
...
call ode_linear_tvp(a0,b0,k,xscheb,chebintr,chebint2,ps,qs,fs,hs,hders,hder2s)
...
subroutine ode_linear_ivp(a,b,k,xscheb,chebintl,chebint2,ps,qs,fs,ys,yders,yder2s)
implicit double precision (a-h,o-z)

integer, intent(in)           :: k
double precision, intent(in)  :: xscheb(k),chebintl(k,k),chebint2(k,k)
...
subroutine ode_linear_tvp(a,b,k,xscheb,chebintr,chebint2,ps,qs,fs,ys,yders,yder2s)
implicit double precision (a-h,o-z)
...
double precision, intent(in)  :: ps(k),qs(k),fs(k)
double precision, intent(out) :: ys(k),yders(k),yder2s(k)
double precision, intent(in)  :: chebint2(k,k)
...
double precision, allocatable :: amatr(:,:),xs(:),sigma(:)
...
subroutine ode_trap_ivp(ier,k,ts,odefun,ys,yders,yder2s,userptr)
implicit double precision (a-h,o-z)
...
subroutine ode_trap_tvp(ier,k,ts,odefun,ys,yders,yder2s,userptr)
implicit double precision (a-h,o-z)
