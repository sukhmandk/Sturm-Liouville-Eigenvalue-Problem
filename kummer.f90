...
!     This version of the routine adaptively discretizes the solution alpha.
!
!   kummer_adap_known - construct the derivative of a nonoscillatory phase
!     function satisfying (2) in the event that its boundary values are
!     known a priori
!
!   kummer_solve - solve Kummer's equation (2) using the window algorithm.
!     This version of the routine requires that the user specify a 
!     discretization scheme for the solution (that is, it is not adaptive).
!
...
!     the user can  control the interval on which the windowing algorithm is 
!     applied.
!
...
subroutine kummerfun(t,val,userptr)
import c_ptr
...
subroutine kummerfun2(t,val,der,userptr)
import c_ptr
implicit double precision (a-h,o-z)
double precision, intent(in)  :: t
double precision, intent(out) :: val,der
type(c_ptr)                   :: userptr
end subroutine
...
type      kummer_data
integer                                :: ifwindowed
double precision                       :: a,b,c,alpha,val0
type(c_ptr)                            :: userptr
procedure(kummerfun), pointer, nopass  :: qfunptr
end type  kummer_data

contains

subroutine odekummer(t,y,yp,f,dfdy,dfdyp,userptr)
implicit double precision (a-h,o-z)

double precision, intent(in)  :: t,y,yp
double precision, intent(out) :: f,dfdy,dfdyp
type(c_ptr)                   :: userptr
type(kummer_data), pointer    :: kdata

call c_f_pointer(userptr,kdata)
call kdata%qfunptr(t,qval,kdata%userptr)

if (kdata%ifwindowed .eq. 1) then
phi  = (erf(kdata%alpha*(t-kdata%c))+1.0d0)/2
qval = kdata%val0*(1.0d0-phi) + phi * qval
endif
...
subroutine odekummer2(t,y,yp,f,dfdy,dfdyp,userptr)
implicit double precision (a-h,o-z)
...
!
!  Set the windowing parameters for use by odekummer.
!
...
kdata%a          = a
kdata%b          = b
kdata%alpha      = 12.0d0/(b-a)
kdata%c          = (a+b)/2
call qfun(kdata%c,kdata%val0,userptr)
kdata%userptr    = userptr
kdata%qfunptr    => qfun
userptr0 = c_loc(kdata)
...
call chebadap(ier,sqrt(eps),a,b,qfun,k,xscheb,ucheb,nints00,ab00,userptr)
if (ier .ne. 0) then
...
call ode_solve_ivp_adap(ier,eps,nints00,ab00,k,xscheb,chebintl,ucheb,nints0,ab0,  &
   alphap,alphapp,alphappp,odekummer,ya,ypa,userptr0)
...
call ode_solve_tvp_adap(ier,eps,nints0,ab0,k,xscheb,chebintr,ucheb,nints,ab,  &
   alphap,alphapp,alphappp,odekummer,yb,ypb,userptr0)
...
subroutine kummer_adap_known(eps,a,b,qfun,k,xscheb,chebintl,chebintr,ucheb, &
   ifleft,ap,app,nints,ab,alphap,alphapp,userptr)
implicit double precision (a-h,o-z)

double precision, intent(in)               :: a,b,ap,app
integer, intent(in)                        :: k,ifleft
integer, intent(out)                       :: nints
double precision, allocatable, intent(out) :: ab(:,:)
double precision,intent(in)                :: xscheb(k),chebintl(k,k),chebintr(k,k),ucheb(k,k)
double precision, allocatable, intent(out) :: alphap(:,:),alphapp(:,:)
...
!  adaptively.
!
!  In this version of the routine, the values of alpha' and alpha'' at either the
!  left or right endpoint of the interval [a,b] must be specified.
!
!  Input parameters:
!    eps - the precision for the calculations
!    (a,b) - the interval on which to compute the phase function
...
!      chebexps
!    ucheb - the values-to-coefficients matrix returned by chebexps
!    ifleft - indicates which side of the interval the values of alpha' and alpha''
!      are specified
!    ap - the value of alpha'
!    app - the value of alpha''
!    userptr - a user-specified c_ptr which is passed to qfun
!   
!  Output parameters:
!    (nints,ab) - the discretization scheme used to represent the solution
!   alphap - a (k,nints) array specifying the values of the derivative of
...
type(kummer_data), pointer      :: kdata
type(c_ptr)                     :: userptr0
double precision, allocatable   :: ab00(:,:),alphappp(:,:)
...
!
!  Indicate that no windowing is to take place
!
...
!
!  Solve Kummer's equation in the appropriate direction
!
...
call ode_solve_ivp_adap(ier,eps,nints00,ab00,k,xscheb,chebintl,ucheb,nints,ab,  &
   alphap,alphapp,alphappp,odekummer,ya,ypa,userptr0)
...
call ode_solve_tvp_adap(ier,eps,nints00,ab00,k,xscheb,chebintr,ucheb,nints,ab,  &
   alphap,alphapp,alphappp,odekummer,yb,ypb,userptr0)
...
subroutine kummer_adap_known3(eps,a,b,qfun,k,xscheb,chebintl,chebintr,ucheb, &
   ifleft,ap,app,nints,ab,alphap,alphapp,alphappp,userptr)
implicit double precision (a-h,o-z)
...
double precision, allocatable, intent(out) :: ab(:,:)
double precision,intent(in)                :: xscheb(k),chebintl(k,k),chebintr(k,k),ucheb(k,k)
double precision, allocatable, intent(out) :: alphap(:,:),alphapp(:,:),alphappp(:,:)
procedure (kummerfun)                      :: qfun
type(c_ptr)                                :: userptr
...
type(kummer_data), pointer      :: kdata
type(c_ptr)                     :: userptr0
double precision, allocatable   :: ab00(:,:)
...
subroutine kummer_adap_known2(eps,a,b,qfun,k,xscheb,chebintl,chebintr,ucheb, &
   ifleft,ap,app,nints,ab,beta,betap,betapp,userptr)
implicit double precision (a-h,o-z)
...
double precision, allocatable, intent(out) :: ab(:,:)
double precision,intent(in)                :: xscheb(k),chebintl(k,k),chebintr(k,k),ucheb(k,k)
double precision, allocatable              :: beta(:,:),betap(:,:),betapp(:,:)
!double precision, allocatable, intent(out) :: alphap(:,:),alphapp(:,:)
procedure (kummerfun)                      :: qfun
type(c_ptr)                                :: userptr
...
call ode_solve_ivp_adap(ier,eps,nints00,ab00,k,xscheb,chebintl,ucheb,nints,ab,  &
   beta,betap,betapp,odekummer2,ya,ypa,userptr0)
...
call ode_solve_tvp_adap(ier,eps,nints00,ab00,k,xscheb,chebintr,ucheb,nints,ab,  &
   beta,betap,betapp,odekummer2,yb,ypb,userptr0)
...
subroutine kummer_adap2(eps,a,b,qfun,k,xscheb,chebintl,chebintr,ucheb, &
   nints,ab,beta,betap,betapp,userptr)
implicit double precision (a-h,o-z)
...
double precision, allocatable, intent(out) :: ab(:,:)
double precision,intent(in)                :: xscheb(k),chebintl(k,k),chebintr(k,k),ucheb(k,k)
double precision, allocatable, intent(out) :: beta(:,:),betap(:,:),betapp(:,:)
procedure (kummerfun)                      :: qfun
type(c_ptr)                                :: userptr
...
call chebadap(ier,eps,a,b,qfun,k,xscheb,ucheb,nints00,ab00,userptr)
if (ier .ne. 0) then
call prini("in kummer_adap, after chebadap, ier = ",ier)
stop
endif
...
subroutine kummer_solve(eps,nints,ab,qfun,k,xscheb,chebintl,chebintr,ucheb,alphap,alphapp, &
  userptr)
implicit double precision (a-h,o-z)
...
double precision,intent(in)                :: xscheb(k),chebintl(k,k),chebintr(k,k),ucheb(k,k)
double precision, intent(out)              :: alphap(k,nints),alphapp(k,nints)
procedure (kummerfun)                      :: qfun
type(c_ptr)                                :: userptr

!
!  Construct a nonoscillatory function alpha' which satisfies Kummer's equation (2).
!  The solution is represented via a discretization scheme specified by the user.
!
!  Input parameters:
!    eps - the precision for the calculations
!    (nints,ab) - the discretization scheme used to represent the solution
...
double precision, allocatable :: alphappp(:,:),ab0(:,:),ab00(:,:)
type(kummer_data), pointer    :: kdata
type(c_ptr)                   :: userptr0
double precision, allocatable :: alphappp0(:,:),alphap0(:,:),alphapp0(:,:)

data pi /3.14159265358979323846264338327950288d0/


allocate(alphappp(k,nints))
...
call ode_solve_ivp(ier,nints,ab,k,xscheb,chebintl,alphap,alphapp,alphappp,odekummer,userptr0)
...
kdata%ifwindowed = 0
call ode_solve_tvp(ier,nints,ab,k,xscheb,chebintr,alphap,alphapp,alphappp,odekummer,userptr0)

if (ier .ne. 0) then
call prini("after ode_solve_tvp, ier = ",ier)
stop
endif
...
subroutine kummer_solve_known(eps,nints,ab,qfun,k,xscheb,chebintl,chebintr,ucheb,alphap,alphapp, &
  userptr,valap,derap)
implicit double precision (a-h,o-z)
...
call chebadap(ier,eps,a,c,qfun,k,xscheb,ucheb,nints00,ab00,userptr)
...
call ode_solve_tvp_adap(ier,eps,nints00,ab00,k,xscheb,chebintr,ucheb,nints1,ab1,  &
   alphap1,alphapp1,alphappp1,odekummer,yb,ypb,userptr0)
...
call chebadap(ier,eps,d,b,qfun,k,xscheb,ucheb,nints00,ab00,userptr)
if (ier .ne. 0) then
call prini("after chebadap, ier = ",ier)
stop
endif
...
call ode_solve_ivp_adap(ier,eps,nints00,ab00,k,xscheb,chebintl,ucheb,nints2,ab2,  &
   alphap2,alphapp2,alphappp2,odekummer,ya,ypa,userptr0)
...
subroutine kummer_phase3(ifleft,k,xscheb,chebintl,chebintr,ucheb, &
   nints,ab,alpha,alphap)
implicit double precision (a-h,o-z)

integer, intent(in)                         :: k,nints,ifleft
double precision, intent(in)                :: ab(2,nints),alphap(k,nints)
double precision, intent(in)                :: xscheb(k),chebintl(k,k),chebintr(k,k),ucheb(k,k)
double precision, allocatable, intent(out)  :: alpha(:,:)
...
subroutine kummer_zero(ifleft,nintsinv,abinv,k,xscheb,alphainv,a1,a2,n,root)
implicit double precision (a-h,o-z)

integer, intent(in)                        :: nintsinv,k,n,ifleft
...
dd   = (dd1*dd)/diff
dd1  = - dd1
sum1 = sum1+dd*alpha(i,int)
sum2 = sum2+dd
sum3 = sum3+dd*alphap(i,int)
dd   = - dd
end do
