...
!   laTeX tables:
!
!     latex_table_init - write out the header for a laTeX table
...
! subroutine cqrsolv(n,a,rhs,sol)
! implicit complex *16 (a-h,o-z)
! double complex         ::  a(n,n),u(2,2),rhs(n),sol(n)
...
! !    rhs - the right-hand side of the system to be solved
! ! 
! !  Output parameters:
...
! allocate(ipiv(n))
! sol = rhs
! call zgesv(n,1,a,n,ipiv,sol,n,info)
! if (info .ne. 0) then
...
! d1=u(1,1)*sol(j-1)+u(1,2)*sol(j)
! d2=u(2,1)*sol(j-1)+u(2,2)*sol(j)
! sol(j-1)=d1
! sol(j)=d2
! end do
! end do
...
! !
! !  Apply the inverse of the triangular matrix to sol
! !
! !call cqrtrin(a,sol,n)
